...
export type MetaCapiBrowserEventName =
  | "PageView"
  | "ViewContent"
  | "SimulationStarted"
  | "ContactFormStarted";
...
type MetaCapiBaseInput = MetaCapiUserInput & {
  eventId: string;
  eventTimeSec: number;
...
export function hashExternalIdForMeta(userId: string): string {
...
function buildUserData(input: MetaCapiUserInput, includeEmail = true) {
  ...
  if (input.fbLoginId?.trim()) userData.fb_login_id = input.fbLoginId.trim();
...
async function postMetaEvent(input: {
  eventName: MetaCapiBrowserEventName | "Lead" | "Contact";
  ...
  const diagnosticBase = input.source
    ? {
        eventName: input.eventName as "Lead" | "Contact",
...
export async function sendMetaCapiContact(input: MetaCapiBaseInput) {
  await postMetaEvent({
    eventName: "Contact",
    input,
    source: "contact",
